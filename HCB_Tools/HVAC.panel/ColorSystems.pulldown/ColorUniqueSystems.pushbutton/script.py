# -*- coding: utf-8 -*-
__title__ = "Color Unique Systems"
__doc__ = """Date  = 06.2025 
_____________________________________________________________________
Komentarz:
Skrypt umożliwia tworzenie filtrów widoku dla systemów kanałów i rur oraz nadpisywanie ich kolorem wypełnienia wg schematu.

Jak korzystać:
-Wybierz filtry które chcesz dodać do widoku
...
view = doc.ActiveView
ordered_vent = list(ventilation_system_colors.keys())
ordered_pipe = list(piping_system_colors.keys())

selected_vent = forms.SelectFromList.show(ordered_vent, multiselect=True, title="Wybierz systemy wentylacyjne") or []
selected_pipe = forms.SelectFromList.show(ordered_pipe, multiselect=True, title="Wybierz systemy rurowe") or []

if selected_vent or selected_pipe:
    t = Transaction(doc, "Nadpisanie systemów")
    t.Start()
    try:
        if selected_vent:
            create_view_filters_and_set_overrides(doc, view, selected_vent, ventilation_system_colors, True)
        if selected_pipe:
            create_view_filters_and_set_overrides(doc, view, selected_pipe, piping_system_colors, False)
        t.Commit()
    except:
        t.RollBack()
        raise